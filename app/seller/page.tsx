...
export default async function SellerDashboardPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-6 pb-20 pt-8 sm:pb-24 sm:pt-12">
      <section className="relative mb-8 overflow-hidden rounded-[2rem] bg-[linear-gradient(118deg,#FFFFFF_0%,#F1F7FF_56%,#ECFEFF_100%)] px-6 py-8 shadow-[0_24px_64px_-46px_rgba(37,99,235,0.42)] ring-1 ring-border/80 sm:mb-10 sm:px-9 sm:py-10"><div className="pointer-events-none absolute -right-14 -top-20 size-64 rounded-full bg-primary/10 blur-2xl" /><div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="sb-eyebrow">Seller dashboard</p><h1 className="sb-heading-xl">{store.name}</h1><p className="mt-3 text-muted-foreground">A clear view of your catalog, orders, and store performance.</p></div><Link href="/seller/products/new" className={buttonVariants({ className: "w-fit rounded-xl" })}>Add product</Link></div></section>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Card className="rounded-[1.35rem] border-0 ring-1 ring-border"><CardHeader><CardDescription className="flex items-center gap-2"><Package className="size-4 text-primary" /> Total products</CardDescription><CardTitle className="text-3xl tracking-[-0.03em]">{productSummary._count._all}</CardTitle></CardHeader></Card>
        <Card className="rounded-[1.35rem] border-0 ring-1 ring-border"><CardHeader><CardDescription className="flex items-center gap-2"><ShoppingCart className="size-4 text-primary" /> Total orders</CardDescription><CardTitle className="text-3xl tracking-[-0.03em]">{totalOrders}</CardTitle></CardHeader></Card>
        <Card className="rounded-[1.35rem] border-0 ring-1 ring-border"><CardHeader><CardDescription className="flex items-center gap-2"><Clock3 className="size-4 text-warning" /> Pending orders</CardDescription><CardTitle className="text-3xl tracking-[-0.03em]">{pendingOrders}</CardTitle></CardHeader></Card>
        <Card className="rounded-[1.35rem] border-0 ring-1 ring-border"><CardHeader><CardDescription className="flex items-center gap-2"><CheckCircle2 className="size-4 text-success" /> Delivered orders</CardDescription><CardTitle className="text-3xl tracking-[-0.03em]">{deliveredOrders}</CardTitle></CardHeader></Card>
        <Card className="rounded-[1.35rem] border-0 bg-[#0f172a] text-white ring-1 ring-slate-800 shadow-[0_20px_48px_-32px_rgba(15,23,42,0.72)]"><CardHeader><CardDescription className="flex items-center gap-2 text-slate-400"><Banknote className="size-4 text-cyan-300" /> Revenue</CardDescription><CardTitle className="text-3xl tracking-[-0.03em] text-white">{formatCurrency(revenue._sum.total ?? 0)}</CardTitle></CardHeader></Card>
        <Card className="rounded-[1.35rem] border-0 ring-1 ring-border"><CardHeader><CardDescription className="flex items-center gap-2"><Warehouse className="size-4 text-primary" /> Total stock</CardDescription><CardTitle className="text-3xl tracking-[-0.03em]">{productSummary._sum.stock ?? 0}</CardTitle></CardHeader></Card>
      </div>
