...
export default async function StorePage({ params, searchParams }: StorePageProps) {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-6 pb-20 pt-8 sm:pb-24 sm:pt-10">
      ...
      <Card className="mb-12 overflow-hidden rounded-[2rem] border-0 py-0 sb-surface">
        {store.banner ? <div className="relative h-44 w-full bg-slate-100 sm:h-60"><Image src={store.banner} alt={`${store.name} banner`} fill unoptimized className="object-cover" sizes="100vw" /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/45 via-slate-950/10 to-transparent" aria-hidden="true" /></div> : <div className="h-32 bg-[linear-gradient(118deg,#0B1224_0%,#132D72_58%,#087A98_125%)] sm:h-40" />}
        <CardContent className="relative flex flex-col gap-5 p-6 sm:flex-row sm:items-center sm:p-8">
          ...
          <div className="self-start rounded-xl bg-[#0f172a] px-4 py-3 text-center text-white shadow-[0_18px_42px_-30px_rgba(15,23,42,0.8)] sm:self-center"><p className="text-2xl font-bold">{total}</p><p className="text-xs font-medium text-slate-400">{total === 1 ? "product" : "products"}</p></div>
        </CardContent>
      </Card>
      ...
      {totalPages > 1 && <nav className="mt-12 flex flex-wrap items-center justify-center gap-3" aria-label="Store products pagination">
